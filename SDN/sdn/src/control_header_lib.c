...
char* create_response_header(int sock_index, uint8_t control_code, uint8_t response_code, uint16_t payload_len)
{
    ...
    getpeername(sock_index, (struct sockaddr *)&addr, &addr_size);
        ...
        /* Control Code */
        memcpy(cntrl_resp_header+0x04, &control_code, sizeof(control_code));
        /* Response Code */
        memcpy(cntrl_resp_header+0x05, &response_code, sizeof(response_code));
        /* Payload Length */
        payload_len = htons(payload_len);
        memcpy(cntrl_resp_header+0x06, &payload_len, sizeof(payload_len));
